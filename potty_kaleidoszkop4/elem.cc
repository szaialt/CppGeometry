...
Elem Elem::rotate(int u, int v, int delta){
  ...
  center_.y = y_;
  color col_ = this->col;
  Elem el = Elem(n, this->R, alpha_, beta_, degrees, center_, col);
  return el;
}
...
Elem Elem::mirror_horizontal(int h){
  ...
  a_.x = x;
  a_.y = y_;
  Elem pol = Elem(n, this->R, -alpha, -beta, degrees, a_, col);
  return pol;
}
...
Elem Elem::mirror_vertical(int w){
  ...
  a_.y = y;
  color col_ = this->col;
  Elem pol = Elem(n, this->R, -alpha, -beta, degrees, a_, col_);
  return pol;
}
