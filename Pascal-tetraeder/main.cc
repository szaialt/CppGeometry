...
vector<vector<int> > multiply(vector<vector<int> > matrix, 
  ...
  for (int i = 0; i < matrix.size(); i++){
    ...
    for (int j = 0; j < matrix.at(i).size(); j++){
      int l = matrix.at(i).at(j) * factors.at(i);
      row.push_back(l);
    }
    mx.push_back(row);
  }
...
vector<vector<vector<int> > > generate_Pascal_tetraeder(int n){
   vector<vector<vector<int> > > tetraeder;
  for (int j = 1; j < n; j++){
     vector<vector<int> > triangle = generate_Pascal_triangle(j);
     int tr = triangle.size();
     vector<int> row = triangle.at(tr-1);
     tetraeder.push_back(triangle);
...
int main(int argc, char *argv[]) {
    ...
    while (SDL_WaitEvent(&ev) && ev.type!=SDL_QUIT && ev.type != SDL_KEYDOWN){
       if (ev.type == SDL_KEYDOWN)
        ...
        cout << size << endl << endl;
        cout << tetraeder.size() << endl << endl;
      for (int i0 = 0; i0 < tetraeder.size(); i0++){
        vector<vector<int> > triangle = tetraeder.at(i0);//
        
        for (int i = 0; i < maxColours-1; i++){
            int offset = width/2;
          
            color col = int_to_color(i, colorMaxValue);
