...
//        }
       ...
       Polygon* elem = new Polygon(symmetry, n, alpha, degrees, 
                              center, b, col);
       //vector<Polygon> polygons = vector<Polygon>();
       //polygon.draw(screen, d);
       //Ide kellenek az elforgatottak/tükrözöttek
       //Miért lesz tele a képernyő?
       int delta = degrees/symmetry;
       elem->draw(screen, R, d, width/2, height/2);
//       for (int i = 0; i < symmetry; i++){
         //Miért nem középre kerül?
//         (elem.rotate(width/2, height/2, delta*i)).draw(screen, R, d);
