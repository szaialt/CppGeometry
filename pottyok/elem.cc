...
vector<point> Elem::elem(int dx, int dy){
  ...
  int phi = alpha + direction;
  vector<point> points = vector<point>();
  for (int i = 0; i <= n; i++){
    int x = center.x + this->a * cos(degrees_to_rads * phi) + dx;
    int y = center.y + this->b * sin(degrees_to_rads * phi) + dy;
    point p;
    p.x = x;
    p.y = y;
    points.push_back(p);
...
void Elem::draw(SDL_Surface *screen, int t){
  ...
  for (int i = 0; i < n; i++){
    filledCircleRGBA(screen, points.at(i).x-t, points.at(i).y, t, col.red, 
