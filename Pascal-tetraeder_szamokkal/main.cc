...
int main(int argc, char *argv[]) {
    ...
    if (ttf_init != 0) {
      cerr << "Nem sikerült a TTF_Init(). " << TTF_GetError() << endl;
     }
      TTF_Font *font;
    ...
    const unsigned long long tileSize = 22;
    ...
    while (SDL_WaitEvent(&ev) && ev.type!=SDL_QUIT && ev.type != SDL_KEYDOWN){
      ...
      //Ezt máshopnnan kellene megtudnunk
        for (unsigned long long i0 = 0; i0 < tetraeder.size(); i0++){
