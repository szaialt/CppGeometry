...
vector<point> Polygon::polygon(double t, int dx, int dy){
  ...
  for (int i = 0; i <= n; i++){
    double x = center.x + r * cos(degrees_to_rads * phi) + dx;
    double y = center.y + r * sin(degrees_to_rads * phi) + dy;
    point p;
    p.x = x;
    p.y = y;
    points.push_back(p);
...
Polygon Polygon::mirror_horizontal(){
  int n_ = this->n;
  int alpha_ = this->alpha * (-1);
  int degrees_ = this->degrees;
  int birth_time_ = this->birth_time;
  point center_;
  center_.x =  this->center.x;
  center_.y = this->center.y * (-1);
  color col_ = this->col;
  Polygon pol = Polygon(n_, alpha_, degrees_, birth_time_, center_, col_);
  return pol;
}

Polygon Polygon::mirror_vertical(){
  int n_ = this->n;
  int alpha_ = this->alpha  * (-1);
  int degrees_ = this->degrees;
  int birth_time_ = this->birth_time;
  point center_;
  center_.x =  this->center.x * (-1);
  center_.y = this->center.y;
  color col_ = this->col;
  Polygon pol = Polygon(n_, alpha_, degrees_, birth_time_, center_, col_);
  return pol;
}
